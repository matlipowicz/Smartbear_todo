...
export const MobileEventModal = ({ closeEventModal, currentEventData }: { closeEventModal: () => void; currentEventData: TaskObjTypes }) => {
    return (
        <div
            className={`block md:hidden absolute top-1/2 left-1/2 -translate-y-1/2 -translate-x-1/2 bg-white rounded-lg shadow dark:bg-gray-300 w-3/4 transition-all duration-500`}
        >
            <div className="relative  flex items-center justify-between p-4 md:p-5 ">
                <h3 className="text-base md:text-xl lg:text-2xl font-semibold text-gray-900 dark:text-white break-words w-1/2">
                    {currentEventData.task_title}
                </h3>
                <div className="flex gap-4 top-2 right-2 absolute">
            ...
            <div className="p-4 flex flex-col gap-6">
                <p className="text-xs md:text-sm lg:text-base font-extralight text-gray-100">{moment(currentEventData.finalDate).format('LLL')}</p>
