        ...
        <li key={task.id} className="mb-4">
            <div className={`flex w-full items-center min-h-taskHeight bg-gray-200 ${isChecked ? 'brightness-50' : ''} rounded-md relative`}>
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <p className={`text-white text-xl break-words break-all w-full mb-2 ${isChecked ? 'line-through' : ''}`}>{task.task_title}</p>
                        <p className="text-gray-100 text-sm break-words">{task.createdAt}</p>
                    </div>
                </div>
